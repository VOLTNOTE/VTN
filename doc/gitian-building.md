...
*Setup instructions for a gitian build of VOLTNOTE using a VM or physical system.*

Gitian is the deterministic build process that is used to build the VOLTNOTE
...
to the VOLTNOTE GitHub release page.
...
- [Building VOLTNOTE](#building-voltnote-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/voltnote-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/voltnote-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/voltnote-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/voltnote-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/voltnote-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/voltnote-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/voltnote-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@voltnote.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/voltnote-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/voltnote-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/voltnote-osx-*-build.assert
```

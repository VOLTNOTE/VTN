...
class ReorgStakeTest(voltnoteTestFramework):
    ...
    def check_money_supply(self, expected_vtn):
        # verify that nodes have the expected VTN supply
        vtn_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(vtn_supply, [DecimalAmt(expected_vtn)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that VTN supply was properly updated after the reorgs
        self.log.info("Check VTN supply...")

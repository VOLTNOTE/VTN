...
CVtnStake* CVtnStake::NewVtnStake(const CTxIn& txin)
{
    ...
    return new CVtnStake(txPrev->vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CVtnStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CVtnStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CVtnStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CVtnStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CVtnStake::GetUniqueness() const
{
    //The unique identifier for a VTN stake is the outpoint
...
const CBlockIndex* CVtnStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CVtnStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CVtnStake::ContextCheck(int nHeight, uint32_t nTime)
{

...
const char * const VOLTNOTE_CONF_FILENAME = "voltnote.conf";
const char * const VOLTNOTE_PID_FILENAME = "voltnote.pid";
const char * const VOLTNOTE_MASTERNODE_CONF_FILENAME = "masternode.conf";


// VOLTNOTE only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\VOLTNOTE
// Windows >= Vista: C:\Users\Username\AppData\Roaming\VOLTNOTE
// Mac: ~/Library/Application Support/VOLTNOTE
// Unix: ~/.voltnote
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VOLTNOTE";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\VOLTNOTEParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\VOLTNOTEParams
    // Mac: ~/Library/Application Support/VOLTNOTEParams
    // Unix: ~/.voltnote-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "VOLTNOTEParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "VOLTNOTEParams";
#else
    // Unix
    return pathRet / ".voltnote-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/voltnote/sapling-spend.params";
        sapling_output = "/usr/share/voltnote/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/voltnote/sapling-spend.params";
            sapling_output = "/usr/local/share/voltnote/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", VOLTNOTE_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", VOLTNOTE_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty voltnote.conf if it does not exist
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override voltnote.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", VOLTNOTE_PID_FILENAME));

...
#if defined(NDEBUG)
#error "VOLTNOTE cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uvtn) are considered zero fee (for relaying, mining and transaction creation)
...
{
    util::ThreadRename("voltnote-scriptch");
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> vtnInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                vtnInputs.push_back(stakeIn);
        }
        const bool hasVTNInputs = !vtnInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasVTNInputs) {
                    ...
                    for (const CTxIn& vtnIn : vtnInputs)
                        if(vtnIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const auto& txIn : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasVTNInputs) {
                            for (const CTxIn& stakeIn : vtnInputs)

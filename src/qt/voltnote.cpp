...
#include "config/voltnote-config.h"
#endif
...
#include "qt/voltnote/splash.h"
#include "qt/voltnote/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("voltnote", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in voltnote.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in voltnote.qrc)
...
/** Class encapsulating VOLTNOTE startup and shutdown.
...
/** Main VOLTNOTE application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (VOLTNOTEGUI)
...
{
    window = new VOLTNOTEGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &VOLTNOTEGUI::detectShutdown);
}
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &VOLTNOTEGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(VOLTNOTEGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(VOLTNOTEGUI::DEFAULT_WALLET);
        ...
        // VOLTNOTE: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &VOLTNOTEGUI::handlePaymentRequest);
        connect(window, &VOLTNOTEGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. VOLTNOTE can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
int main(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(voltnote_locale);
    Q_INIT_RESOURCE(voltnote);
    ...
    /// 6. Determine availability of data directory and parse voltnote.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("VOLTNOTE"),
    ...
    try {
        ...
        QMessageBox::critical(0, QObject::tr("VOLTNOTE"), QObject::tr("Error: %1").arg(e.what()));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // voltnote: links repeatedly have their payment requests routed to this process:

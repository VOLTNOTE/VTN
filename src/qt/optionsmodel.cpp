...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::VTN);
    ...
    if (settings.contains("nAnonymizevoltnoteAmount") || reset)
        gArgs.SoftSetArg("-anonymizevoltnoteamount", settings.value("nAnonymizevoltnoteAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in voltnote.cpp during shotdown to also remove the window positions

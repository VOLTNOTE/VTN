...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter VOLTNOTE address (e.g. %1)").arg("VaTpABdcNhyP2RNVzfZzyGHbGIcgbHC4RC"));
...
{
    // return if URI is not valid or is no VOLTNOTE: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::VTN, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert voltnote:// to voltnote:
    //
    //    Cannot handle this later, because voltnote:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::VTN, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    if (gArgs.GetBoolArg("-testnet", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "VOLTNOTE (testnet).lnk";
    else if (gArgs.GetBoolArg("-regtest", false))
        return GetSpecialFolderPath(CSIDL_STARTUP) / "VOLTNOTE (regtest).lnk";

    return GetSpecialFolderPath(CSIDL_STARTUP) / "VOLTNOTE.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for VOLTNOTE*.lnk
...
{
    return GetAutostartDir() / "voltnote.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart) {
        ...
        if (!optionFile.good())
            return false;
        // Write a voltnote.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (gArgs.GetBoolArg("-testnet", false))
            optionFile << "Name=VOLTNOTE (testnet)\n";
        else if (gArgs.GetBoolArg("-regtest", false))
            optionFile << "Name=VOLTNOTE (regtest)\n";
        else
            optionFile << "Name=VOLTNOTE\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the voltnote app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add voltnote app to startup item list

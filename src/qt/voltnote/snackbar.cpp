...
#include "qt/voltnote/forms/ui_snackbar.h"
#include "qt/voltnote/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(VOLTNOTEGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &VOLTNOTEGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {

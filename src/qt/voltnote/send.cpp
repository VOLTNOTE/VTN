...
#include "qt/voltnote/send.h"
#include "qt/voltnote/forms/ui_send.h"
#include "qt/voltnote/addnewcontactdialog.h"
#include "qt/voltnote/qtutils.h"
#include "qt/voltnote/sendchangeaddressdialog.h"
#include "qt/voltnote/optionbutton.h"
#include "qt/voltnote/sendconfirmdialog.h"
#include "qt/voltnote/guitransactionsutils.h"
#include "qt/voltnote/loadingdialog.h"
...
SendWidget::SendWidget(VOLTNOTEGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-vtn");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onVTNSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onVTNSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations(CAmount delegationBalance)
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent VTNs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield VTNs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent VTNs to shield."));
    }
}
...
void SendWidget::onVTNSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto vtnAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(vtnAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(vtnAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(vtnAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {

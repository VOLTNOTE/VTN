...
#include "qt/voltnote/dashboardwidget.h"
#include "clientmodel.h"
#include "guiutil.h"
#include "optionsmodel.h"
#include "qt/voltnote/forms/ui_dashboardwidget.h"
#include "qt/voltnote/qtutils.h"
#include "qt/voltnote/sendconfirmdialog.h"
#include "qt/voltnote/txrow.h"
#include "utiltime.h"
...
DashboardWidget::DashboardWidget(VOLTNOTEGUI* parent) : PWidget(parent),
                                                                       ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareVtn, "square-chart-vtn");
    setCssProperty(ui->labelVtn, "text-chart-vtn");
    ...
    setCssProperty(ui->labelAmountVtn, "text-stake-vtn-disable");
    ...
    if (window)
        connect(window, &VOLTNOTEGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("VTN")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair VTN
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            vtn = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalVtn += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesVtn.append(vtn);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(46, 148, 33));
    ...
    set0->append(chartData->valuesVtn);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalVtn > 0) {
        setCssProperty(ui->labelAmountVtn, "text-stake-vtn");
    } else {
        setCssProperty(ui->labelAmountVtn, "text-stake-vtn-disable");
    }
    forceUpdateStyle({ui->labelAmountVtn});
    ui->labelAmountVtn->setText(GUIUtil::formatBalance(chartData->totalVtn, nDisplayUnit));

...
#include "qt/voltnote/forms/ui_addresseswidget.h"
#include "qt/voltnote/addresslabelrow.h"
#include "qt/voltnote/addnewaddressdialog.h"
#include "qt/voltnote/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(VOLTNOTEGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto vtnAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(vtnAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(vtnAdd);
        if (walletModel->updateAddressBookLabels(vtnAdd, label.toUtf8().constData(),

...
#include "qt/voltnote/settings/settingsfaqwidget.h"
#include "qt/voltnote/qtutils.h"
#include "qt/voltnote/settings/forms/ui_settingsfaqwidget.h"
#include <QMetaObject>
#include <QScrollBar>

SettingsFaqWidget::SettingsFaqWidget(VOLTNOTEGUI* parent) : QDialog(parent),
                                                                           ui(new Ui::SettingsFaqWidget)
{
    ...
    setCssProperty({ui->labelSubtitle_Intro,
                       ui->labelSubtitle_UnspendableVTN,
    ...
    setCssProperty({ui->labelContent_Intro,
                       ui->labelContent_UnspendableVTN,
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("VOLTNOTE is a form of digital online money using blockchain technology "
               "that can be easily transferred globally, instantly, and with near "
               "zero fees. VOLTNOTE incorporates market leading security & "
               "privacy and is also the first PoS (Proof of Stake) Cryptocurrency "
               "to implement Sapling(SHIELD), a zk-SNARKs based privacy protocol.")) +
        formatFAQParagraph(
            tr("VOLTNOTE utilizes a Proof of Stake (PoS) consensus system algorithm, "
               "allowing all owners of VOLTNOTE to participate in earning block rewards "
               "while securing the network with full node wallets, as well as to "
               "run Masternodes to create and vote on proposals.")));
    ui->labelContent_Intro->setText(introContent);

    QString unspendableVTNContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received VOLTNOTE requires 6 confirmations on the network "
               "to become eligible for spending which can take ~12 to ~15 minutes.")) +
        formatFAQParagraph(
            tr("Your VOLTNOTE wallet also needs to be completely synchronized "
               "to see and spend balances on the network.")));
    ui->labelContent_UnspendableVTN->setText(unspendableVTNContent);
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of VOLTNOTE with a minimum of 60 confirmations.")) +
        ...
        formatFAQParagraph(
            tr("You can see the status of staking in the wallet by mousing over the "
               ...
               "will confirm that staking is active.")
                .arg("voltnote", "<span style=\"font-style:italic\">getstakingstatus</span>")));
    ui->labelContent_Stake->setText(stakeContent);

    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("For Support you can contact us on our Email: support@voltnote.info")));
    ui->labelContent_Support->setText(supportContent);

    QString masternodeContent = formatFAQContent(
        formatFAQParagraph(
            tr("A masternode is a computer running a full node VOLTNOTE wallet with a "
               "requirement of 1,000 VTN secured collateral to provide extra services "
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in VOLTNOTE Governance")) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the VOLTNOTE network")))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("1,000 VTN per single Masternode instance")) +
    ...
    QString mNControllerContent = formatFAQContent(
        formatFAQParagraph(
            tr("A Masternode Controller wallet is where the 1,000 VTN collateral "
    ...
    connect(ui->pushButton_UnspendableVTN, &QPushButton::clicked, [this]() { onFaqClicked(ui->widget_UnspendableVTN); });
    ...
    if (parent)
        connect(parent, &VOLTNOTEGUI::windowResizeEvent, this, &SettingsFaqWidget::windowResizeEvent);
}

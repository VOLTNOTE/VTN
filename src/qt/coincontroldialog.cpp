...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select VTN Outputs to Spend" :
            "Select Shielded VTN to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::VTN;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show VOLTNOTE address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {

...
bool ProcessBlockFound(const std::shared_ptr<const CBlock>& pblock, CWallet& wallet, Optional<CReserveKey>& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("VOLTNOTEMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, nullptr, pblock, nullptr)) {
        return error("VOLTNOTEMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("VOLTNOTEMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("voltnote-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running VOLTNOTEMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("VOLTNOTEMiner exception");
    } catch (...) {
        LogPrintf("VOLTNOTEMiner exception");
    }

    LogPrintf("VOLTNOTEMiner exiting\n");
}

...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/VOLTNOTEParams"
    else
        PARAMS_DIR="$HOME/.voltnote-params"
    fi
fi
...
Failed to fetch the VOLTNOTE zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
VOLTNOTE - fetch-params.sh

This script will fetch the VOLTNOTE zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common VOLTNOTE zkSNARK parameters. Note that it is
